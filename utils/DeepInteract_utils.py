...
from utils.deepinteract_constants import FEAT_COLS, ALLOWABLE_FEATS, D3TO1
from utils.dips_plus_utils import postprocess_pruned_pairs, impute_postprocessed_missing_feature_values
from utils.graph_utils import prot_df_to_dgl_graph_feats
from utils.protein_feature_utils import GeometricProteinFeatures
...
def recover_any_missing_chain_ids(interim_dataset_dir: str, new_pdb_filepath: str,
    ...
    with open(pair_filenames[0], 'rb') as f:
        pair = dill.load(f)
    # Update the corresponding chain ID
    # pair.df0_o = copy.deepcopy(pair.df0)
    # pair.df1_o = copy.deepcopy(pair.df1)
...
def convert_input_pdb_files_to_pair(left_pdb_filepath: str, right_pdb_filepath: str, input_dataset_dir: str,
                                    psaia_dir: str, psaia_config: str, hhsuite_db: str, fast=False):
    ...
    if not fast:
        ...
        os.makedirs(os.path.join(input_dataset_dir, "interim", "external_feats", pdb_code, "work"), exist_ok=True)
